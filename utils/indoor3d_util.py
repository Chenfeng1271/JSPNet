...
def collect_point_label(anno_path, out_filename, file_format='txt'):
    ...
    for f in glob.glob(os.path.join(anno_path, '*.txt')):
        ...
        if cls not in g_classes:  # note: in some room there is 'staris' class..
            cls = 'clutter'
        points = np.loadtxt(f)
        labels = np.ones((points.shape[0], 1)) * g_class2label[cls]#semantic class2label    
        instancelabels = np.ones((points.shape[0], 1)) * instanceid
        instanceid += 1
        points_list.append(np.concatenate([points, labels, instancelabels], 1))  # Nx8 这边怎么会有8的
...
#visiualize the label using color mask
...
# -----------------------------------------------------------------------------
def num_room2blocks_wrapper_normalized(data_label_filename, block_size=1.0, stride=1.0,
...
def room2blocks(data, label, inslabel, num_point, block_size=1.0, stride=1.0,
    ...
    for idx in range(len(xbeg_list)):
        ...
        del block_data_sampled
        del block_label_sampled
        del block_inslabel_sampled
